 class TickerPanel {
   ...
   addInitialSymbols() {
    ...
    // Загружаем только сохранённые символы
    savedSymbols.forEach(symbolKey => {
        const parts = symbolKey.split(':');
        if (parts.length === 3) {
            const [symbol, exchange, marketType] = parts;
            this.addSymbol(symbol, true, exchange, marketType, false);
        }
    });
    ...
    // Загружаем начальные данные через REST
    Promise.all([
...
clearAllSymbols() {
  ...
  setupModal() {
    ...
    // ✅ ИСПРАВЛЕННЫЙ ОБРАБОТЧИК keydown
    modalSearch.addEventListener('keydown', (e) => {
        const input = e.target; // ссылка на поле ввода
        const code = e.code;

        // Словарь кодов клавиш → английские символы (строчные)
        const codeToEng = {
            ...
            'Backquote': '`', 'Digit0': '0', 'Digit1': '1', 'Digit2': '2', 'Digit3': '3',
            'Digit4': '4', 'Digit5': '5', 'Digit6': '6', 'Digit7': '7', 'Digit8': '8', 'Digit9': '9'
        };

        // Если клавиша есть в словаре – перехватываем и вставляем английский символ
        if (codeToEng.hasOwnProperty(code)) {
            e.preventDefault();
            let char = codeToEng[code];
        ...
        // Для служебных клавиш (Backspace, Enter, стрелки и т.д.) ничего не делаем
    });

    modalSearch.addEventListener('input', (e) => {
        e.target.value = e.target.value.toUpperCase();
        this.state.modalSearchQuery = e.target.value;
        this.state.modalPage = 0;
  ...
  addNextBatch() {
    ...
    for (let i = start; i < end; i++) {
        ...
        if (symbolData && symbolData.symbol && !this.tickers.some(t => 
            ...
            this.addSymbol(symbolData.symbol, true, symbolData.exchange, symbolData.marketType, false);
        }
    }
    ...
    if (end < allPairs.length) {
        ...
        // ✅ ПОСЛЕ ДОБАВЛЕНИЯ ВСЕХ СИМВОЛОВ - ЗАГРУЖАЕМ НАЧАЛЬНЫЕ ДАННЫЕ
        this.loadInitialDataForAllSymbols();
        
        this.setupWebSockets();
    }
}

// ✅ НОВЫЙ МЕТОД: загрузка начальных данных для всех символов
async loadInitialDataForAllSymbols() {
    console.log('🔄 Загрузка начальных данных для всех символов...');
    
    const symbolsToLoad = this.tickers.filter(t => t.price === 0 || t.price === null);
    
    for (const ticker of symbolsToLoad) {
        await this.fetchInitialDataForSymbol(ticker.symbol, ticker.exchange, ticker.marketType);
        // Небольшая задержка, чтобы не забанили
        await new Promise(resolve => setTimeout(resolve, 100));
    }
    
    console.log(`✅ Загружены начальные данные для ${symbolsToLoad.length} символов`);
    this.debouncedUpdatePrices();
}
 ...
 _processBinanceTicker(tickerData, exchange, marketType) {
    ...
    if (ticker) {
        ...
        if (tickerData.c !== undefined) {
            ...
            // Проверка на значительное изменение (более 0.5%)
            if (ticker.price !== 0 && ticker.price !== undefined) {
                const changePercent = Math.abs((newPrice - ticker.price) / ticker.price) * 100;
                if (changePercent > 0.5) {
                    hasSignificantChange = true;
                    if (this.debugMode) {
                        console.log(`⚠️ Значительное изменение ${symbol}: ${ticker.price} → ${newPrice} (${changePercent.toFixed(2)}%)`);
                    }
                }
            }
            
            // Обновление текущей свечи на графике (если это активный символ)
            if (window.chartManagerInstance && window.chartManagerInstance.currentSymbol === symbol) {
                window.chartManagerInstance.updateCurrentCandle(newPrice);
            }
        }
        ...
        // Сохраняем флаг значительного изменения для немедленного обновления
        if (hasSignificantChange) {
            this._hasSignificantChange = true;
        }
        
        // Логирование получения тикера (для отладки)
        if (this.debugMode && (Math.random() < 0.05)) { // Логируем только 5% тикеров, чтобы не засорять консоль
            console.log(`📊 Binance ticker: ${symbol} price=${tickerData.c} change=${tickerData.P}%`);
        }
    } else if (this.debugMode && (Math.random() < 0.01)) {
        // Логируем пропущенные тикеры (не в нашем списке) только иногда
        console.log(`⏭️ Пропущен тикер (не в списке): ${symbol}`);
    }
}
 applyBinanceUpdates() {
    ...
    // ✅ Устраняем гонку данных: создаём локальную копию и сразу очищаем очередь
    ...
    updates.forEach((update, key) => {
        ...
        if (ticker) {
            ...
            if (update.change !== undefined) {
                ticker.change = update.change;
            }
            if (update.volume !== undefined) {
                ticker.volume = update.volume;
            }
            if (update.trades !== undefined) {
                ticker.trades = update.trades;
            }
            hasUpdates = true;
        }
    });
    ...
    if (hasUpdates) {
        // ✅ Для значительных изменений обновляем немедленно
        if (hasSignificantChange) {
            if (this.debugMode) {
                console.log(`⚡ Немедленное обновление панели из-за значительного изменения`);
            }
            this.updatePriceImmediate();
        } else {
            this.debouncedUpdatePrices();
  ...
  connectBybitWebSocket(marketType, url) {
    ...
    update.volume = parseFloat(tickerData.turnover24h);
            ...
            if (!this.updatePending) {
                this.updatePending = true;
                setTimeout(() => this.applyBybitUpdates(), 100);
            }
        } catch (error) { 
...
async fetchBybitSpotPrices(symbols) {
  ...
  updatePriceElements() {
    ...
    // ✅ Если были обновления и сортировка по объёму, пересортировываем
...
if (this.totalItems === 0) {
    ...
    formatPrice(price) {
        ...
        if (this.formatCache.prices.has(cacheKey)) {
            return this.formatCache.prices.get(cacheKey);
        }
        ...
        this.formatCache.prices.set(cacheKey, result);
        return result;
    }

    formatChange(change) {
        if (change === undefined || change === null) return '0.00';
        const cacheKey = change.toFixed(2);
        if (this.formatCache.changes.has(cacheKey)) {
            return this.formatCache.changes.get(cacheKey);
        }
        const result = (change > 0 ? '+' : '') + change.toFixed(2);
        this.formatCache.changes.set(cacheKey, result);
        return result;
    }
...
formatVolume(volume) {
    ...
    if (volume < 1) return volume.toFixed(4); // ✅ Для малых объёмов показываем знаки
  ...
  addSymbol(symbol, isCustom = true, exchange = 'binance', marketType = 'futures', render = true) {
  ...
  if (exchange === 'bybit') {
    this.updateBybitSubscription(marketType, symbol);  // ← ПРАВИЛЬНО
}
