class WebSocketManager {
  ...
  connectKline(symbol, interval, exchange, marketType) {
    ...
    if (this.wsKline) {
        this.wsKline.onclose = null; this.wsKline.onerror = null; this.wsKline.onmessage = null;
    ...
    this.wsKline.onopen = () => console.log('✅ Trade (свечи) открыт');
    this.wsKline.onmessage = (event) => {
        ...
        const step = { '1m':60,'3m':180,'5m':300,'15m':900,'30m':1800,'1h':3600,'4h':14400,'6h':21600,'12h':43200,'1d':86400,'1w':604800,'1M':2592000 }[interval] || 3600;
        const aligned = Math.floor(nowSec / step) * step;

        if (aligned > last.time) {
            const next = { time: aligned, open: price, high: price, low: price, close: price, volume: 0 };
            cm.chartData.push(next);
            cm.lastCandle = next;
            (cm.currentChartType === 'candle' ? cm.candleSeries : cm.barSeries)?.update(next);
        } else {
            last.close = price;
            if (price > last.high) last.high = price;
            if (price < last.low) last.low = price;
            cm.lastCandle = last;
            (cm.currentChartType === 'candle' ? cm.candleSeries : cm.barSeries)?.update(last);
        }

        // === МЕНЯЕМ ЦВЕТ ЛИНИИ ЦЕНЫ В ЗАВИСИМОСТИ ОТ СВЕЧИ ===
    ...
    this.wsKline.onclose = () => {
        this.wsKline = null;
        if (this.currentSymbol === symbol) this.klineReconnectTimer = setTimeout(() => this.connectKline(symbol, interval, exchange, marketType), 3000);
    };
