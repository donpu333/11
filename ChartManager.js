...
getCurrentPrice() {
    ...
    // 3. Самый последний резерв — цена закрытия последней свечи
    if (this.lastCandle && this.lastCandle.close) {
...
// ========== МЕТОД ДЛЯ ОБНОВЛЕНИЯ ФОРМАТА ЦЕНЫ ==========
// ========== МЕТОД ДЛЯ ОБНОВЛЕНИЯ ФОРМАТА ЦЕНЫ ==========
...
async fetchKlines(symbol, exchange, marketType, interval, limit = 200) {
    const bybitIntervalMap = {
        '1m': '1', '3m': '3', '5m': '5', '15m': '15', '30m': '30',
        '1h': '60', '4h': '240', '6h': '360', '12h': '720',
        '1d': 'D', '1w': 'W', '1M': 'M'
 ...
 setChartType(type) {
    ...
    // ✅ ДОБАВЬТЕ ЭТО - объём всегда виден
