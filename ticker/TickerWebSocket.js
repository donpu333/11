class TickerWebSocket {
    constructor(parent) {
        ...
        // ✅ УВЕЛИЧИЛ ДЕБАУНС С 50 ДО 300ms
        this.wsDebounceMs = 300;
    ...
    refreshSubscriptions() {
        console.log('🔄 Обновление WebSocket подписок...');
        
        const bybitFuturesWs = this.wsConnections.find(c => 
            c.url && c.url.includes('linear') && c.readyState === WebSocket.OPEN
        );
        ...
        if (bybitFuturesWs) {
            ...
            if (bybitFuturesSymbols.length > 0) {
                ...
                bybitFuturesWs.send(JSON.stringify(subscribeMsg));
                console.log(`📡 Обновлена подписка Bybit Futures: ${bybitFuturesSymbols.length} символов`);
            }
        }
        
        const bybitSpotWs = this.wsConnections.find(c => 
            c.url && c.url.includes('spot') && c.url.includes('v5') && c.readyState === WebSocket.OPEN
        );
        ...
        if (bybitSpotWs) {
            ...
            if (bybitSpotSymbols.length > 0) {
                ...
                bybitSpotWs.send(JSON.stringify(subscribeMsg));
                console.log(`📡 Обновлена подписка Bybit Spot: ${bybitSpotSymbols.length} символов`);
            }
        }
    }
    ...
    // ✅ ИСПРАВЛЕННЫЙ МЕТОД - НАКАПЛИВАЕТ ОБНОВЛЕНИЯ
    _processBinanceTicker(tickerData, exchange, marketType) {
        ...
        if (!this.updateBatch.has(key)) {
            this.updateBatch.set(key, {});
        }
        const update = this.updateBatch.get(key);
        
        if (tickerData.c !== undefined) update.price = parseFloat(tickerData.c);
    ...
    // ✅ НОВЫЙ МЕТОД - ПРИМЕНЯЕТ ПАКЕТ ОБНОВЛЕНИЙ
    flushBatch() {
        if (this.updateBatch.size === 0) return;
        
        for (const [key, update] of this.updateBatch.entries()) {
            const ticker = this.parent.tickersMap.get(key);
            if (ticker) {
                if (update.price !== undefined) ticker.price = update.price;
    ...
    forceReconnect() {
        ...
        setTimeout(() => {
            console.log('🔌 Перезапуск WebSocket соединений...');
