class TickerRenderer {
    ...
    loadMoreTickers() {
        ...
        const currentCount = this.parent.tickers.length;
        const totalAvailable = this.parent.allSymbolsCache?.length || 0;
        
        if (currentCount >= totalAvailable || totalAvailable === 0) return;
        
        this.isLoadingMore = true;
        
        const nextBatch = this.parent.allSymbolsCache.slice(
            currentCount, 
            Math.min(currentCount + this.batchSize, totalAvailable)
        );
        
        console.log(`📥 Подгружаем еще ${nextBatch.length} тикеров...`);
        
        nextBatch.forEach(s => {
            const key = `${s.symbol}:${s.exchange}:${s.marketType}`;
            if (!this.parent.tickersMap.has(key)) {
                this.parent.addSymbol(s.symbol, true, s.exchange, s.marketType, false, true);
            }
        });
        
        this.parent.state.customSymbols = this.parent.tickers.map(t => 
            `${t.symbol}:${t.exchange}:${t.marketType}`
        );
        ...
        console.log(`✅ Теперь тикеров: ${this.parent.tickers.length}`);
    }
    ...
    renderTickerList() {
        ...
        this._scrollHandler = () => {
            ...
            const visibleElements = Math.ceil(containerHeight / this.rowHeight);
            const lastVisibleIndex = Math.floor(scrollTop / this.rowHeight) + visibleElements;
            const remaining = this.totalItems - lastVisibleIndex;
            
            if (remaining < this.loadThreshold && !this.isLoadingMore) {
                const totalAvailable = this.parent.allSymbolsCache?.length || 0;
                if (this.parent.tickers.length < totalAvailable) {
                    this.loadMoreTickers();
                }
            }
        };
    ...
    formatTrades(ticker) {
        ...
        const cached = this.tradesCache?.get(ticker.trades);
        if (cached) return cached;
        
        let result;
        if (ticker.trades > 1e9) result = (ticker.trades / 1e9).toFixed(1) + 'B';
        else if (ticker.trades > 1e6) result = (ticker.trades / 1e6).toFixed(1) + 'M';
        else if (ticker.trades > 1e3) result = (ticker.trades / 1e3).toFixed(1) + 'K';
        else result = ticker.trades.toString();
        
        if (!this.tradesCache) this.tradesCache = new Map();
        this.tradesCache.set(ticker.trades, result);
