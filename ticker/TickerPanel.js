...
this.watchlists = new TickerWatchlists(this);
...
this.cacheTTL = 30000;
  ...
  addInitialSymbols() {
   ...
   clearAllSymbols() {
    if (this.watchlists) {
        const active = this.watchlists.getActive();
        
        if (!active) {
            console.warn('Нет активного вотчлиста');
            return;
        }
        
        if (active.id === 'default') {
            if (!confirm('Очистить основной вотчлист? Все символы будут удалены.')) {
                return;
            }
        }
        
        // Очищаем символы в активном вотчлисте
        active.symbols = [];
        this.watchlists.saveWatchlists();
        ...
        this.state.flags = {};
        if (this.tickerElements) this.tickerElements.clear();
        ...
        this.saveState();
        this.watchlists.renderUI();
        this.renderTickerList();
        
        // ✅ ПЕРЕПОДКЛЮЧАЕМ WEBSOCKET ЧЕРЕЗ forceReconnect
        if (this.ws && this.ws.forceReconnect) {
            this.ws.forceReconnect();
        } else if (this.setupWebSockets) {
            setTimeout(() => this.setupWebSockets(), 100);
        }
        
        this.showNotification(`🗑 Очищен вотчлист "${active.name}"`, '#f23645');
        ...
        this.showNotification('🗑 Все символы удалены', '#f23645');
    }
}

// Добавь вспомогательный метод для уведомлений (если нет)
showNotification(message, color = '#4CAF50') {
    const notification = document.getElementById('alertNotification');
    if (notification) {
        notification.innerHTML = `
            <div class="alert-title">📋 Вотчлист</div>
            <div class="alert-price">${message}</div>
        `;
    ...
    addSymbol(symbol, isCustom = true, exchange = 'binance', marketType = 'futures', render = true, skipInitialFetch = false) {
        ...
        if (this.tickersMap.has(key)) return false;
          if (this.watchlists) {
        const active = this.watchlists.getActive();
        if (active && !active.symbols.includes(key)) {
            // Добавляем через watchlists, он сам синхронизирует
            return this.watchlists.addSymbol(symbol, exchange, marketType);
        }
    }
        const newTicker = { 
            symbol, 
...
async addSymbolsBatch(symbolsData) {
    ...
    // Быстрая проверка через Set для уникальности
    const existingKeys = new Set(this.tickersMap.keys());
    
    for (const { symbol, exchange, marketType } of symbolsData) {
        if (!symbol) continue;
        
        const symbolClean = symbol.trim().toUpperCase();
        if (!symbolClean.endsWith('USDT')) continue;
        
        const key = `${symbolClean}:${exchange}:${marketType}`;
        ...
        const symbolKey = `${symbolClean}:${exchange}:${marketType}`;
        if (!this.state.customSymbols.includes(symbolKey)) { 
            this.state.customSymbols.push(symbolKey); 
        }
        
        addedSymbols.push({ symbol: symbolClean, exchange, marketType });
    }
    ...
    // Параллельная загрузка цен
    await Promise.all([
        this.fetchBatchSnapshots(addedSymbols),
        // Можно добавить другие параллельные задачи
    ]);
    ...
    // WebSocket подключаем без задержки
    if (this.setupWebSockets) {
        this.setupWebSockets();
    }
}
...
async fetchBatchSnapshots(symbols) {
    ...
    // Bybit — разделяем спот и фьючерсы для скорости
                    ...
                    for (const ticker of data.result.list) {
                        if (symbolSet.has(ticker.symbol)) {
                            ...
                            if (tickerObj) {
                                tickerObj.price = parseFloat(ticker.lastPrice);
                                tickerObj.change = parseFloat(ticker.price24hPcnt) * 100;
                                tickerObj.volume = parseFloat(ticker.turnover24h);
                            }
                        }
                    }
                })
                .catch(e => console.warn('Bybit Futures batch error:', e))
        );
    }
    ...
    if (bybitSpot.length > 0) {
        promises.push(
            fetch('https://api.bybit.com/v5/market/tickers?category=spot')
                ...
                .catch(e => console.warn('Bybit Spot batch error:', e))
        );
    }
    ...
    // Один вызов обновления цен после всех загрузок
    this.debouncedUpdatePrices();
}
