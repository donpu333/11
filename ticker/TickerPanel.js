  ...
  addInitialSymbols() {
    ...
    fetch('https://fapi.binance.com/fapi/v1/ticker/24hr')
        ...
        .then(data => {
            ...
            this.renderTickerList();
            console.log('✅ Binance Futures загружены через REST');
        })
        .catch(e => console.error('❌ Ошибка Binance Futures:', e));
    ...
    fetch('https://api.binance.com/api/v3/ticker/24hr')
        ...
        .then(data => {
            ...
            this.renderTickerList();
            console.log('✅ Binance Spot загружены через REST');
        })
        .catch(e => console.error('❌ Ошибка Binance Spot:', e));
                ...
                data.result.list.forEach(t => {
                    this.tickersMap.forEach(ticker => {
                        if (ticker.symbol === t.symbol && ticker.exchange === 'bybit' && ticker.marketType === 'futures') {
                            ...
                            ticker.volume = parseFloat(t.volume24h);
                            ticker.trades = 0;
                        }
                    });
                });
                this.renderTickerList();
                console.log('✅ Bybit Futures загружены через REST');
            }
        })
        .catch(e => console.error('❌ Ошибка Bybit Futures:', e));
    ...
    fetch('https://api.bybit.com/v5/market/tickers?category=spot')
        ...
        .then(data => {
            if (data.retCode === 0 && data.result?.list) {
                ...
                this.renderTickerList();
                console.log('✅ Bybit Spot загружены через REST');
            }
        })
        .catch(e => console.error('❌ Ошибка Bybit Spot:', e));
    
    // Обновляем цены каждую секунду через REST
    if (this._priceInterval) clearInterval(this._priceInterval);
    this._priceInterval = setInterval(() => {
        // Binance (фьючерсы + спот)
   ...
   clearAllSymbols() {
    ...
    async fetchBatchSnapshots(symbols) {
        ...
        if (binanceSpot.length > 0 || binanceFutures.length > 0) {
            promises.push(
                fetch('https://fapi.binance.com/fapi/v1/ticker/24hr')
                    .then(r => r.json())
                    .then(data => {
                        const allBinance = [...binanceSpot, ...binanceFutures];
                        const symbolSet = new Set(allBinance.map(s => s.symbol));
                        data.forEach(ticker => {
