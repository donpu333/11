class TickerModal {
    ...
    if (e.key === 'Enter' && modal.classList.contains('show')) {
        ...
        if (firstItem) {
            ...
            let added = false;
            if (this.parent.watchlists) {
                added = this.parent.watchlists.addSymbol(symbol, exchange, marketType);
            } else {
                added = this.parent.addSymbol(symbol, true, exchange, marketType);
            }
 ...
 async addNextBatch() {
    ...
    for (let i = start; i < end; i++) {
        ...
        if (symbolData && symbolData.symbol) {
            // Проверяем через watchlists
            let isAdded = false;
            if (this.parent.watchlists) {
                const activeWatchlist = this.parent.watchlists.getActive();
                const key = `${symbolData.symbol}:${symbolData.exchange}:${symbolData.marketType}`;
                isAdded = activeWatchlist?.symbols.includes(key);
            } else {
                isAdded = this.parent.tickers.some(t => 
    ...
    // ✅ ИСПРАВЛЕНО: добавляем через watchlists если есть
    if (batchToAdd.length > 0) {
        if (this.parent.watchlists && this.parent.watchlists.addSymbolsBatch) {
            await this.parent.watchlists.addSymbolsBatch(batchToAdd);
        } else {
